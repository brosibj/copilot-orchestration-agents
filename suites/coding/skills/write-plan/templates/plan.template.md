...
## Definition of Done
- [ ] Build command succeeds per the active project instructions, with warnings no worse than `Build Baseline` in `research.md`
- [ ] Test results are no worse than `Build Baseline` in `research.md`; any unchanged pre-existing failures are called out in `report.md`
