...
- **Nested subagents** — orchestrators now may use [nested subagents](https://code.visualstudio.com/docs/copilot/agents/subagents#_nested-subagents) to fan out across a research topic, divide-and-conquer implementation, or recursive delegation. Requires enablement of `chat.subagents.allowInvocationsFromSubagents` via vscode settings; user will be prompted to do so when running `/align-project` for the first time.
